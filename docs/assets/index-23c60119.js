 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */const Et="important",xe=" !"+Et,we=ye(class extends Ce{constructor(e){var t;if(super(e),e.type!==$e.ATTRIBUTE||e.name!=="style"||((t=e.strings)===null||t===void 0?void 0:t.length)>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(e){return Object.keys(e).reduce((t,a)=>{const i=e[a];return i==null?t:t+`${a=a.includes("-")?a:a.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${i};`},"")}update(e,[t]){const{style:a}=e.element;if(this.ht===void 0){this.ht=new Set;for(const i in t)this.ht.add(i);return this.render(t)}this.ht.forEach(i=>{t[i]==null&&(this.ht.delete(i),i.includes("-")?a.removeProperty(i):a[i]="")});for(const i in t){const n=t[i];if(n!=null){this.ht.add(i);const r=typeof n=="string"&&n.endsWith(xe);i.includes("-")||r?a.setProperty(i,r?n.slice(0,-11):n,r?Et:""):a[i]=n}}return L}});function l(e){const{vertices:t,size:a,fill:i,label:n,transforms:r,children:s=!1}=e;return Te({children:s,vertices:t,size:a,fill:i,label:n,transforms:r})}let Ot=function(e){return Math.PI*e/180},Ae=function(e,t,a){return a+e*Math.cos(Ot(t))},Se=function(e,t,a){return a+e*Math.sin(Ot(t))},vt=function(e){return e<1e-7?0:e};const Te=e=>{let t=e.size||96,a=t/2,i=1*t/2,n=360/e.vertices,r=90,s=e.fill||"red",p=[0,0,t,t].join(" ");const v=[function(m){for(var f=[],b=0;b<m;b++){var w=n*b-r,z=[b===0?"M":"L",vt(Ae(i,w,a)),vt(Se(i,w,a))].join(" ");f.push(z)}return f.join(" ")}(e.vertices)].join(" ");return e.children?o`
        ...
        `:o` ${Q(t,p,s,v,e.transforms)} `};function Q(e,t,a,i,n){let r={pointerEvents:"none",transformOrigin:"center",transform:`${n??""}`};return x`<svg
    viewBox=${t}
    width=${e}
    height=${e}
    fill=${a}
  ...
  >
    <path d=${i} />
  </svg>`}function kt(e=100){return l({vertices:22,size:e,fill:"currentColor"})}function It(){return o`<h2 class="headline" />`}function d(e=1,t){return o` ${[...Array(e)].map(a=>o`<p class="text ${t?"text-short":""}" />`)}`}function tt(){return o` <button>${d()}</button> `}function _e(e){return{form:{vertices:5},utils:{vertices:7},media:{vertices:6},theme:{vertices:3},group:{vertices:4}}[e]}function ft(e){const t=document.querySelector(e);t instanceof HTMLElement&&(t.innerHTML="")}function T(e){return document.querySelector(e)}function Pt(e){return[].slice.call(document.querySelectorAll(e))}const Ee=["chevronLeft","chevronRight","chevronDown","chevronUp","caretLeft","caretRight","caretDown","caretUp","arrowDown","arrowUp","slash","handle","check","cross","plus","dotsY","dotsX","minus","heart","bookmark","pin","star"];function Oe(){return o`
  ...
  `}function ke(e){_(Ie(e),document.querySelector("article"))}function Ie(e){const{label:t,snippets:a}=e;return o`
    ...
    `}function Pe(e){const{value:t,label:a,kind:i,description:n}=e,{kind:r}=h.getState(),s=p=>{if(r!==p){g(h,"SET_KIND",p);const c=X.filter(v=>v.kind===p);g(h,"FILTERED",c)}else g(h,"SET_KIND",void 0),g(h,"FILTERED",X)};return t===""?o`
            <li data-list-item="">
                <div>
    ...
    ${d(2)}
        ...
        `}function et(){const e=Pt('input[type="checkbox"]:not(#checkbox-e)'),t=Array.from(e);return{checked:t.filter(a=>a.checked),all:t}}function Me(){const{output:e}=h.getState(),t=(n=!0)=>{const r=T("#checkbox-e");r!=null&&(r.indeterminate=n)},a=()=>{const{checked:n,all:r}=et();n.length===0?(g(h,u.SET_OUTPUT,r),r.filter(s=>!s.disabled).map(s=>s.checked=!0)):(g(h,u.SET_OUTPUT,[]),r.filter(s=>!s.disabled).map(s=>s.checked=!1))},i=()=>{const{checked:n}=et();if(n.length===0){t(!1);return}g(h,u.SET_OUTPUT,n),t()};return o`
        ${d(4)}
    ...
    `}function Be(){return o`
        ...
        ${d(2)}
    `}function He(){const e=Pt('input[type="radio"]:not(#radio-d)'),t=Array.from(e);return{checked:t.filter(a=>a.checked),all:t}}function Ge(){const{output:e}=h.getState(),t=()=>{const{checked:a}=He();g(h,u.SET_OUTPUT,a)};return o`
        ${d(2)}
    ...
    `}function Ve(){const e=t=>{t.target.scrollIntoView({behavior:"smooth",inline:"center",block:"center"})};return o`
        ...
        ${d(1)}
    `}function je(){const{output:e}=h.getState(),t=i=>{const n=i.target.selectedIndex+1;g(h,u.SET_OUTPUT,[{defaultValue:`Option ${n}`}])},a=i=>{var s;const n=i.target.selectedOptions,r=(s=Array.from(n))==null?void 0:s.map(p=>({defaultValue:`Option ${p.index+1}`}));g(h,u.SET_OUTPUT,r)};return o`
        ${d(7)}
        ...
        <div data-intrinsic-grid data-gap="xl">
            ...
            <div data-group="grid" data-gap="sm">
                <output
                    ...
                    data-border-shadow="outer"
                >
                    ${e.length>0?e.map(i=>o`<p data-font="xl">${i.defaultValue}</p>`):"-"}</output
                >
            </div>
        </div>
        ${d(3)}
    `}function qe(){const{output:e}=h.getState(),t=()=>{const{checked:a}=et();g(h,u.SET_OUTPUT,a)};return o`
        ${d(2)}
    ...
    `}function Fe(){return o`
        ...
        ${d(5)}
    `}function Ze(){const e=t=>{const a=t.target.dataset.tabId,i=document.querySelectorAll("[data-tabs] [data-tab-id]");i&&(i[0].dataset.tabId===a?i[0].dataset.tabVisible="true":i[0].dataset.tabVisible="false",i[1].dataset.tabId===a?i[1].dataset.tabVisible="true":i[1].dataset.tabVisible="false")};return o` ${d(2)}
        ...
        <div
            ...
            data-border-shadow="outer"
        >
            <div data-tab-id="1">${kt(84)} ${d(1)}${d(1,!0)}</div>
            <div data-tab-id="2" data-tab-visible="false">
                ${d(1,!0)} ${d(3)}${d(1,!0)}
            </div>
        </div>

        ${d(2)}`}const Nt=()=>{const e=T("body"),t=T("[data-tray]"),a=T("[data-tray-overlay]");t.dataset.trayOpen==="false"?(a.dataset.trayOverlayOpen="true",t.dataset.trayOpen="true",e.style.overflow="hidden",_(Ke(),T("[data-tray]"))):(a.dataset.trayOverlayOpen="false",t.dataset.trayOpen="false",e.style.overflow="auto")};function We(){return o`
        ${d(1)}
        <button
            id="show-tray"
    ...
    >
        ...
        ${d(1)}
    `}function gt(){return o` <p data-font>Docs incoming</p>
        <div data-paper-color="" data-paper="error">${d(4)}</div>`}function Qe(){return o`
        ${d(1)}
        ...
        ${d(1)}
    `}function Xe(e){const t=document.querySelector("article");t!=null&&_(Je(e),t)}function Je(e){switch(e){case"reset":case"tokens":return o`${gt()}`;case"button":return o`${Le()}`;case"checkbox":return o`${Me()}`;case"input":return o`${Ue()}`;case"radio":return o`${Ge()}`;case"range":return o`${De()}`;case"select":return o`${je()}`;case"progress":return o`${Qe()}`;case"switch":return o`${qe()}`;case"avatar":case"scroller":case"image":return o`${Ve()}`;case"article":case"hero":return o`${Ne()}`;case"icon":return o`${Oe()}`;case"action-sheet":return o`${Re()}`;case"panel":case"card":return;case"group":return o`${Ye()}`;case"list":return o`<p data-font>Check landing</p>
                ${d(4)}`;case"table":return o`${Fe()}`;case"tabs":return o`${Ze()}`;case"tray":return o`${We()}`;case"lobotomized-owl":case"border":case"fit":case"gap":case"gradient":case"hide":case"margin-auto":case"mask":case"overflow":case"padding":case"place":case"print":case"ratio":case"reorder":case"util":return o`${gt()}`;case"font":return o`${ze()}`;case"paper":return o`${Be()}`;default:return o``}}function ta(){return o`
        <picture>
            <img
                src="https://images.unsplash.com/34/BA1yLjNnQCI1yisIZGEi_2013-07-16_1922_IMG_9873.jpg?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1471&q=80"
            />
        </picture>
    `}function ea(e){return o`
    ...
    `}const C={};C.Pic=ta;C.Content=ea;C.Avatar=kt;C.HeadLine=It;C.Line=d;C.Actions=aa;function ia(e,t){return o`
    ...
    `}function na(e){return o`
        <div data-reorder="${e?"top":""}">
            ${d(1,!0)}
            <div>${tt()}${tt()}</div>
        </div>
    `}const S={};S.Content=ia;S.HeadLine=It;S.Line=d;S.Actions=na;function Lt(e){_(ra(e),document.querySelector("aside"))}function ra(e){const{orientation:t}=h.getState(),a=t==="portrait";return e?o``:o`
        <div
            data-group=""
            data-gap="md"
    ...
    `}function Mt(){_(oa(),document.querySelector("article")),zt(),Lt(!0)}function oa(){return o`
        <div id="controls">
            <h2></h2>
            <nav></nav>
        </div>
        <div data-group="" data-panel="">${sa()}</div>
    `}function sa(){const{variantPanel:e}=h.getState();switch(e){case"actions":return o` ${S.Content()} ${S.Actions()} `;case"actions-reorder":return o` ${S.Content()}${S.Actions(!0)} `;case"input":return o` ${S.Content(!0,!0)} ${S.Actions()} `;default:return o` ${S.Content()}`}}function la(){const t=new URLSearchParams(window.location.search).get("snippet"),a=t==="card",i=n=>{const r=n.target.dataset.option;if(t===r)return;const s=new URL(document.location.href);s.searchParams.set("snippet",r),history.pushState({},"",s),r==="card"?Ut():Mt()};return o`
        <div class="subnav bi">
            <button
                data-side="bottom"
                data-nav="group"
    ...
    `}function mt(e,t){const a=n=>{const r=Number(n.target.value);if(r===0)return g(h,u.SET_VARIANT,"pic");if(r===5)return g(h,u.SET_VARIANT,"pic-content");if(r===10)return g(h,u.SET_VARIANT,"pic-reorder");if(r===15)return g(h,u.SET_VARIANT,"actions")},i=n=>({pic:0,"pic-content":5,"pic-reorder":10,actions:15})[n];return t?o``:o`
        <label>
            <input
                type="range"
                min="0"
    ...
    `}function bt(e,t){const a=n=>{switch(Number(n.target.value)){case 0:return g(h,u.SET_VARIANT_PANEL,"actions");case 10:return g(h,u.SET_VARIANT_PANEL,"actions-reorder");case 20:default:return g(h,u.SET_VARIANT_PANEL,"input")}},i={actions:0,"actions-reorder":5,input:10}[e];return t?o``:o`
        <label>
            <input
                type="range"
                min="0"
    ...
    `}function $t(e,t){const a=e==="square",i=e==="portrait",n=e==="landscape",r=s=>{const{nav:p,option:c}=s.target.dataset;p==="ratio"&&g(h,u.SET_ORIENTATION,c)};return t?o``:o`
        <div class="subnav">
            <button
                data-nav="ratio"
                data-option="square"
    ...
    `}const da=[{label:"default",value:"currentColor"},{label:"red",value:"var(--red-100)"},{label:"purple",value:"var(--purple-100)"},{label:"gray",value:"var(--gray-100)"}];function ca(){_(ua(),document.querySelector("section > header div:first-of-type"))}function ua(){const e=t=>{const{swatch:a}=t.target.dataset,i={red:"error",purple:"focus",gray:"gray",default:""}[a],n=T("article"),r=T('header [data-input=""]');n!=null&&r!=null&&(n.classList.remove("error","focus","gray"),r.classList.remove("error","focus","gray"),i!==""&&(n.classList.add(i),r.classList.add(i)))};return o`
        <div data-group="flex" data-gap="sm">
            ${da.map(t=>o`
                    <button
    ...
    `}function zt(){_(pa(),document.querySelector("#controls nav")),_(ha(),document.querySelector("#controls h2"))}function pa(){const{variant:e,variantPanel:t,orientation:a}=h.getState(),n=new URLSearchParams(window.location.search).get("snippet"),r=n==="card",s=n==="panel";return o`
        ${la()} ${r?mt(e):mt(e,!0)}
        ${r?$t(a):$t(a,!0)}
        ${s?bt(t):bt(t,!0)}
    `}function ha(){const t=new URLSearchParams(window.location.search).get("snippet"),a=t==="card"?"content with ratio":"interactive elements";return o` <strong>${`${t}:`}</strong> groups ${a}`}function Ut(){_(Bt(),document.querySelector("article")),zt(),Lt(!1)}function Bt(){const{orientation:e}=h.getState();return o` <div
        class="${e==="portrait"?"fix-portrait-card-height":""}"
    >
        <div data-group="" data-card="" data-ratio="${e}">
            ${va()}
        </div>
    </div>`}function va(){const{variant:e}=h.getState();switch(e){case"actions":return o` ${C.Content()} ${C.Actions()} `;case"pic":return o` ${C.Pic()}
                <div class="pin-avatar">${C.Avatar()}</div>`;case"pic-content":return o` ${C.Pic()} ${C.Content()} `;case"pic-reorder":return o` ${C.Pic()} ${C.Content(!0)} `;default:return o``}}function fa(){_(ga(),document.querySelector("#search-results-container"))}function ga(){const{suggestions:e}=h.getState();return e.length===0?o``:o`
    ...
    `}h.subscribe(Ht);function ma(){const e=T("#q");e!=null&&(e.value=""),g(h,u.FILTERED,q.filtered),g(h,u.SUGGESTIONS,[]),g(h,u.SET_OUTPUT,[])}function ba(e){var i;const a=((i=e.target)==null?void 0:i.value).replace(/[^a-zA-Z]/g,"").toLowerCase();if(a!==""){const n=q.filtered.filter(r=>r.value.startsWith(a));n.length>0?(g(h,u.FILTERED,n),g(h,u.SUGGESTIONS,n)):(g(h,u.FILTERED,q.filtered),g(h,u.SUGGESTIONS,[]))}else g(h,u.SUGGESTIONS,[])}function Ht(){const{filtered:e}=h.getState(),a=new URLSearchParams(window.location.search).get("snippet");if(fa(),a===null){ft("#controls"),ke({label:"Featured snippets",snippets:e});return}ft("h1");const i=T("#controls");["card","panel"].includes(a)?(i!=null&&i.classList.add("show"),a==="card"&&Ut(),a==="panel"&&Mt()):(i!=null&&i.classList.remove("show"),Xe(a),["paper"].includes(a)||ca())}document.addEventListener("DOMContentLoaded",function(){const e=T("#clear-input");e!=null&&e.addEventListener("click",ma);const t=T("#q");t!=null&&t.addEventListener("input",ba),Ht()});
